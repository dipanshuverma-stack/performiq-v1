...
export async function getReadinessEngine(
  userId: string
): Promise<ReadinessEngine> {
  const [
    preparation,
    performance,
  ] = await Promise.all([
  ...
  const readiness = Math.round(
    preparation.readiness *
      READINESS_WEIGHTS.preparation +
      performance.score *
        READINESS_WEIGHTS.performance
  );
  ...
  return {
    ...
    completionScore:
      preparation.completionScore,

    mockScore:
      preparation.mockScore,

    revisionScore:
      preparation.revisionScore,

    unresolvedMistakes:
      preparation.unresolvedMistakes,
