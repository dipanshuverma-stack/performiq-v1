  ...
  return (
    ...
    >
        ...
        <div className="text-left">
          ...
          <div className="mt-3 flex flex-wrap gap-2">
            {tags.map((tag) => (
              <span 
                key={tag} 
                className="rounded-md border border-white/[0.08] bg-white/[0.05] px-2.5 py-1 text-[11px] font-medium text-slate-300 transition-colors duration-200 group-hover:bg-white/[0.07] group-hover:text-white"
              >
                {tag}
              </span>
            ))}
      ...
      <div className="flex items-center gap-3">
        ...
        <ChevronRight className="h-5 w-5 text-slate-500 group-hover:translate-x-1 group-hover:text-indigo-400 transition-all" />
      </div>
    </button>
  );
}