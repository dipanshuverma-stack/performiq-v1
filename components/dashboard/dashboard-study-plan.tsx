...
export const DashboardStudyPlan = React.memo(function DashboardStudyPlan({
  planItems = [],
}: DashboardStudyPlanProps) {
  if (planItems.length === 0) {
    return (
      <section className="space-y-3">
        <SectionHeader title="Today's Study Plan" />
        <GlassCard className="p-12 text-center">
          <p className="text-slate-400">No study plan generated yet.</p>
          <p className="text-sm text-slate-500 mt-1">
            Complete more practice sessions to build your personalized plan.
          </p>
        </GlassCard>
      </section>
    );
          ...
          >
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
              ...
              <div className="flex items-center justify-between sm:justify-end gap-3 shrink-0">
                ...
                <ActionButton
                  size="sm"
                  className="flex items-center gap-1.5 px-4 group/btn"
                >
