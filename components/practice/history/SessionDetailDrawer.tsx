...
export function SessionDetailDrawer({ session, onClose }: DrawerProps) {
  ...
  // ✅ Escape key trap handling plus focus restoration pipeline
  useEffect(() => {
    if (!session) return;
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-6 text-sm">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="bg-slate-50 p-3 rounded-xl border border-slate-100">
                  ...
                  <strong className="text-xl font-bold text-slate-800">{session.qpm} QPM</strong>
                </div>
              </div>
              ...
              <div className="border-t border-slate-100 pt-4 space-y-2">
                ...
                <div className="flex items-center gap-4 text-xs font-medium">
                  <div className="flex items-center gap-1.5 text-emerald-600 bg-emerald-50 border border-emerald-100 px-3 py-1.5 rounded-lg w-full justify-center">
                    🟢 <strong className="font-bold">{session.correctQuestions}</strong> Correct
                  </div>
                  <div className="flex items-center gap-1.5 text-rose-600 bg-rose-50 border border-rose-100 px-3 py-1.5 rounded-lg w-full justify-center">
                    ❌ <strong className="font-bold">{session.incorrectQuestions}</strong> Wrong
                  </div>
                </div>
              </div>
              ...
              <div className="border-t border-slate-100 pt-4 space-y-3">
                ...
                <div className="flex items-center justify-between py-1.5 border-b border-slate-50">
                  <span className="text-slate-500 font-medium">Complexity Level</span>
                ...
                <div className="flex items-center justify-between py-1.5">
                  ...
                  <span className={`text-xs font-bold px-2.5 py-1 rounded-md border ${
                    ...
                    {session.revisionStatus || "UNRESOLVED"}
                  </span>
                </div>
              </div>
              ...
              <div className="border-t border-slate-100 pt-4 space-y-2">
                ...
                <div className="bg-slate-50 p-4 rounded-xl text-xs leading-relaxed text-slate-600 italic border border-slate-100 whitespace-pre-wrap">
                  {session.notes?.trim() || "No customized analytical observation logs appended to this run session."}
                </div>
              </div>
            </div>
